...
RoutingMessage::RoutingMessage() : 
	m_MessageId( "" ), m_BatchId( "" ), m_CorrelationId( "" ), m_SessionId( "" ), m_UserId( "" ), m_Priority( 5 ), m_BatchSequence( 0 ), 
	...
	m_Fastpath( false ), m_Bulk( false ), m_Virtual( false ), m_TableName( "" ), m_Payload( NULL ), m_PayloadEvaluator( NULL ), m_OriginalMessageType( "" ), m_DelayedReplyId( "" )
{
}
...
RoutingMessage::RoutingMessage( const string& tableName, const string& messageId, bool delayRead ) : 
	...
	m_TableName( tableName ), m_Payload( NULL ), m_PayloadEvaluator( NULL ), m_OriginalMessageType( "" ), m_DelayedReplyId( "" )
{
	if ( delayRead )
		return;
	else
...
RoutingMessage::RoutingMessage( const RoutingMessage& source ) : 
	...
	m_TableName( source.getTableName() ), m_Payload( NULL ), m_PayloadEvaluator( NULL ), m_OriginalMessageType( source.getOriginalMessageType() ), m_DelayedReplyId( source.getDelayedReplyId() )
{
	try
	{
...
void RoutingMessage::Read()
{
	...
	try
	{
		...
		m_MessageId = StringUtil::Trim( myData->getCellValue( 0, "GUID" )->getString() );
...
bool RoutingMessage::isNack()
{
	...
	{
		DEBUG( "QPICODE not found. This message is not a reject." );
	}
	...
	if( m_PayloadEvaluator == NULL )
	{
		...
		if ( m_PayloadEvaluator != NULL )
		{
			...
			if ( m_PayloadEvaluator->isBusinessFormat() )
				m_OriginalMessageType = m_PayloadEvaluator->getField( InternalXmlPayload::MESSAGETYPE );
