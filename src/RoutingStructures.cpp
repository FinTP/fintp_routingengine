...
string RoutingExitpoint::ProcessMessage( RoutingMessage* message, const string& userId, bool bulk, const string& batchType )
{
	...
	try
	{
		if ( !isReply )
		{
			...
			// operation is safe because batchId is generated in qPI on assemble operation
			...
			try
			{
				...
				for( unsigned int i=0; i<batchItemsSize; i++ )
				{
					...
					if ( isReply )
					{
						...
						string messageRef = StringUtil::Trim( batchItems->getCellValue( i, "TRN" )->getString() );
					...
					else
					{
						...
						else
						{
							...
							batchItem.setMessageId( StringUtil::Trim( batchItems->getCellValue( i, "GUID" )->getString() ) );
							...
							batchItem.setBatchTotalCount( batchItems->getCellValue( i, "BATCHCOUNT" )->getLong() );
