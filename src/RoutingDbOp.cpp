	...
	const short holdstatus, const long sequence, const string& feedback )
{
	...
	data->ExecuteNonQueryCached( DataCommand::SP, "ModifyMessageInQueue", params );
...
void RoutingDbOp::InsertBusinessMessage( const string& messageType, const string& senderApp, const string& receiverApp, const string& messageId, const string& correlationId, const vector<string>& keywords, const vector<string>& keywordValues )
{
	Database* data = getData();
	...
	keywordsParam->setName( "Keywords" );
	...
	const string& edToEdId, const string& orgInstrId, const string& orgTxId, const string& addMsgInf )
{
	...
	DEBUG_GLOBAL( "Inserting business message [" << messageId << "]" );	
	...
	correlIdParam->setDimension( correlationId.length() );
	correlIdParam->setString( correlationId );
	correlIdParam->setName( "Correlation_id" );
	params.push_back( correlIdParam );
	
	DataParameterBase *crtQueueParam = m_DatabaseProvider->createParameter( DataType::LONGINT_TYPE );
	crtQueueParam->setLong( crtQueue );
	crtQueueParam->setName( "Current_queue" );
	params.push_back( crtQueueParam );
	
	DataParameterBase *messageTypeParam = m_DatabaseProvider->createParameter( DataType::CHAR_TYPE );
	messageTypeParam->setDimension( messageType.length() );
	messageTypeParam->setString( messageType );
	messageTypeParam->setName( "Message_type" );
	params.push_back( messageTypeParam );
	
	DataParameterBase *senderBICParam = m_DatabaseProvider->createParameter( DataType::CHAR_TYPE );
	senderBICParam->setDimension( senderBIC.length() );
	senderBICParam->setString( senderBIC );
	senderBICParam->setName( "Sender BIC" );
	params.push_back( senderBICParam );
	
	DataParameterBase *receiverBICParam = m_DatabaseProvider->createParameter( DataType::CHAR_TYPE );
	receiverBICParam->setDimension( receiverBIC.length() );
	receiverBICParam->setString( receiverBIC );
	receiverBICParam->setName( "Receiver BIC" );
	params.push_back( receiverBICParam );
	
	DataParameterBase *currDateParam = m_DatabaseProvider->createParameter( DataType::CHAR_TYPE );
	currDateParam->setDimension( currDate.length() );
	currDateParam->setString( currDate );
	currDateParam->setName( "Currency date" );
	params.push_back( currDateParam );
	
	DataParameterBase *currTypeParam = m_DatabaseProvider->createParameter( DataType::CHAR_TYPE );
	currTypeParam->setDimension( currType.length() );
	currTypeParam->setString( currType );
	currTypeParam->setName( "Currency" );
	params.push_back( currTypeParam );
	
	DataParameterBase *currAmmountParam = m_DatabaseProvider->createParameter( DataType::CHAR_TYPE );
	currAmmountParam->setDimension( currAmmount.length() );
	currAmmountParam->setString( currAmmount );
	currAmmountParam->setName( "Ammount" );
	params.push_back( currAmmountParam );
	...
	DataParameterBase *trnParam = m_DatabaseProvider->createParameter( DataType::CHAR_TYPE );
	trnParam->setDimension( trn.length() );
	trnParam->setString( trn );
	trnParam->setName( "TRN" );
	params.push_back( trnParam );
	
	DataParameterBase *relrefParam = m_DatabaseProvider->createParameter( DataType::CHAR_TYPE );
	relrefParam->setDimension( relref.length() );
	relrefParam->setString( relref );
	relrefParam->setName( "Related reference" );
	params.push_back( relrefParam );
	
	DataParameterBase *murParam = m_DatabaseProvider->createParameter( DataType::CHAR_TYPE );
	murParam->setDimension( mur.length() );
	murParam->setString( mur );
	murParam->setName( "MUR" );
	params.push_back( murParam );

	if ( RoutingEngine::hasIBANForLiquidities() )
	{
		DataParameterBase *ibanParam = m_DatabaseProvider->createParameter( DataType::CHAR_TYPE );
		ibanParam->setDimension( iban.length() );
		ibanParam->setString( iban );
		ibanParam->setName( "IBAN" );
		params.push_back( ibanParam );
	}
	if ( RoutingEngine::hasIBANPLForLiquidities() )
	{
		DataParameterBase *ibanplParam = m_DatabaseProvider->createParameter( DataType::CHAR_TYPE );
		ibanplParam->setDimension( ibanpl.length() );
		ibanplParam->setString( ibanpl );
		ibanplParam->setName( "IBANPL" );
		params.push_back( ibanplParam );
	}
	if ( RoutingEngine::hasCorrForLiquidities() )
	{
		DataParameterBase *scorrParam = m_DatabaseProvider->createParameter( DataType::CHAR_TYPE );
		scorrParam->setDimension( senderCorr.length() );
		scorrParam->setString( senderCorr );
		scorrParam->setName( "SenderCorr" );
		params.push_back( scorrParam );

		DataParameterBase *rcorrParam = m_DatabaseProvider->createParameter( DataType::CHAR_TYPE );
		rcorrParam->setDimension( receiverCorr.length() );
		rcorrParam->setString( receiverCorr );
		rcorrParam->setName( "ReceiverCorr" );
	...
	DataParameterBase* endToEndIdParam = m_DatabaseProvider->createParameter( DataType::CHAR_TYPE );
	endToEndIdParam->setDimension( edToEdId.length() );
	endToEndIdParam->setString( edToEdId );
	endToEndIdParam->setName( "EndToEndId" );
	params.push_back( endToEndIdParam );

	DataParameterBase* orgInstrIdParam = m_DatabaseProvider->createParameter( DataType::CHAR_TYPE );
	orgInstrIdParam->setDimension( orgInstrId.length() );
	orgInstrIdParam->setString( orgInstrId );
	orgInstrIdParam->setName( "OrgInstrId" );
	params.push_back( orgInstrIdParam );

	DataParameterBase* orgTxIdParam = m_DatabaseProvider->createParameter( DataType::CHAR_TYPE );
	orgTxIdParam->setDimension( orgTxId.length() );
	orgTxIdParam->setString( orgTxId );
	orgTxIdParam->setName( "OrgTxId" );
	params.push_back( orgTxIdParam );

	DataParameterBase* addMsgInfParam = m_DatabaseProvider->createParameter( DataType::CHAR_TYPE );
	addMsgInfParam->setDimension( addMsgInf.length() );
	addMsgInfParam->setString( addMsgInf );
	addMsgInfParam->setName( "MSGINFO" );
	params.push_back( addMsgInfParam );
	// build sp name
	data->ExecuteNonQueryCached( DataCommand::SP, "InsertBusinessMessage", params );
}
...
void RoutingDbOp::UpdateOriginalBatchMessages( const string& batchid, const string& code )
{
	...
	data->ExecuteNonQuery( DataCommand::SP, "UpdateFeedbackaggTFDCode", params );	
}
...
void RoutingDbOp::GetQueues()
{
	...
	try
	{
		...
		for ( unsigned int i=0; i<result->size(); i++ )
		{
			long queueId = result->getCellValue( i, "GUID" )->getLong();
			...
			serviceName = StringUtil::Trim( result->getCellValue( i, "FRIENDLYNAME" )->getString() );
			serviceId = result->getCellValue( i, "SERVICEID" )->getLong();

			// compatibility check ( don't fail if the getqueues doesn't return duplicateq )
			try
			{
				duplicateQueue = StringUtil::Trim( result->getCellValue( i, "DUPLICATEQ" )->getString() );
			}
			catch( ... )
			{
				duplicateQueue = "";
			...
			try
			{
				workColumn = result->getCellValue( i, "DUPLICATENOTIFQ" );
				if( workColumn != NULL )
			...
			try
			{
				workColumn = result->getCellValue( i, "DELAYEDNOTIFQ" );
				if( workColumn != NULL )
