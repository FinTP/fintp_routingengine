...
RoutingEngine::RoutingEngine() : m_CotThreadId( 0 ), m_RouterThreadId( 0 ), m_BMThreadId( 0 ), m_Watcher( ( AbstractWatcher::NotificationPool* )NULL ),
	m_LiquiditiesHasIBAN( false ), m_LiquiditiesHasIBANPL( false ), m_LiquiditiesHasCorresps( false ), m_DuplicateDetectionActive( false ),
	m_DuplicateDetectionTimeout( 0 ), m_IdleArchivingActive( false ), m_Running( true ), m_BulkReactivateQueue( "" ), m_TrackMessages( false )
{
#ifdef SCROLL_CURSOR
...
void RoutingEngine::Start( bool startWatcher )
{
	...
	GlobalSettings.loadConfig( "RoutingEngine.config" );
...
void* RoutingEngine::COTMonitor( void* data )
{
	...
	while( !RoutingEngine::m_ShouldStop )
	{
		...
		sleep( RoutingEngine::m_CotDelay );
	}
