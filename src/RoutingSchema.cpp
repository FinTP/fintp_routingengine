...
void RoutingSchema::LoadSchema( const string& schemaName, long schemaId, const string& sessionCode )
{
	...
	try
	{
		...
		for( unsigned int i=0; i<rules->size(); i++ ) 
		{
			...
			long crtRuleId = rules->getCellValue( i, "GUID_RULE" )->getLong();
...
void RoutingSchema::Load()
{
	...
	try
	{
		...
		for( unsigned int i=0; i<schemas->size(); i++ )
		{
			...
			long schemaId = schemas->getCellValue( i, "GUID" )->getLong();
	...
	else
	{
		...
		try
		{
			...
			for( unsigned int i=0; i<batchItems->size(); i++ )
			{
				...
				{
					//the sp doesn't exist in all versions of qPay.
				}
	...
	else // if it is rapid batch
	{
		...
		try
		{
			do
			{
				...
				string batchId = ( messagesInBatch > 0 ) ? StringUtil::Trim( batchItems->getCellValue( 0, "GUID" )->getString() ) : "";
				string batchRef = ( messagesInBatch > 0 ) ? StringUtil::Trim( batchItems->getCellValue( 0, "BATCHREF" )->getString() ) : "";

				for( unsigned int j=0; j<messagesInBatch; j++ )
				{
					string messageId = StringUtil::Trim( batchItems->getCellValue( j, "GUID" )->getString() );
...
void RoutingSchema::ApplyRouting( RoutingJob* job, RoutingMessage* theMessage, RoutingMessage ( *messageProviderCallback )(), bool fastpath )
{
	...
	if ( job->isComplete() )
	{
		...
		DEBUG( "Performing manual reject of the message with feedback [" << feedback << "]" );
	...
	if( delayId.length() > 0 && delayedTable.length() > 0 )
	{
		try
		{
			...
			{	
				...
				string msgId = StringUtil::Trim( delayedItems->getCellValue( 0, "GUID" )->getString() );
