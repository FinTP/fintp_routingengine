...
void RoutingAction::internalPerformComplete( RoutingMessage* message ) const
{
	if ( m_Param.length() > 0 )
	{
		...
		// reset param to requestor's input queue, a reply will be sent to app
		// send reply
		internalPerformSendReply( message, RoutingDbOp::GetReplyQueue( message->getRequestorService() ) );
	}
	else
	{
...
void RoutingAction::internalInsertBMInfo( RoutingMessage* message )
{
	...
	if( !message->isReply() )
	{
		...
		try
		{
			RoutingDbOp::InsertBusinessMessage( messageType,
		...
		catch( const std::exception& ex )
		{
			...
			RoutingExceptionMoveInvestig aex = internalPerformMoveToInvestigation( message, "Could not obtain all business data from message." );
	...
	try
	{
		...
		*batchManager << batchHeader;
		...
		for ( unsigned int i=0; i<batchSet->size(); i++ )
		{
			...
			batchItem.setMessageId( StringUtil::Trim( batchSet->getCellValue( i, "GUID" )->getString() ) );
...
void RoutingAction::internalPerformEnrichMessage( RoutingMessage* message ) const
{
	...
	catch( const runtime_error& rex)
	{
		...
		internalPerformMoveToInvestigation( message, rex.what() );
	}
	catch( ... )
	{
		...
		internalPerformMoveToInvestigation( message, errorMessage );
	}
}
...
void EnrichTemplate::enrich( RoutingMessage* message )
{
	...
	// update message to qPI
	RoutingAggregationCode businessCode( RoutingMessageEvaluator::AGGREGATIONTOKEN_CORRELID, message->getCorrelationId() );	
	for( int i = 0; i < ENRICHFIELDSCOUNT; i++ )
	{
		int field = m_EnrichFields[i];
		switch ( field )
		{
			case InternalXmlPayload::CURRENCY :
			{
				string currency = m_MessageEvaluator->getField( field );
				businessCode.addAggregationField( "CURRENCYTYPE", m_MessageEvaluator->EvaluateKeywordValue( m_MessageEvaluator->getField( InternalXmlPayload::MESSAGETYPE ), currency, "Currency" ) );
				businessCode.addAggregationField( "CURRENCYAMOUNT", m_MessageEvaluator->EvaluateKeywordValue( m_MessageEvaluator->getField( InternalXmlPayload::MESSAGETYPE ), currency, "Currency" ) );
				break;
			}
			case InternalXmlPayload::VALUEDATE :
			{
				string valueDate = m_MessageEvaluator->getField( field );
				businessCode.addAggregationField( m_EnrichFieldsName[i], m_MessageEvaluator->EvaluateKeywordValue( m_MessageEvaluator->getField( InternalXmlPayload::MESSAGETYPE ), valueDate, "ValueDate" ) );
				break;
			}
			default:
				businessCode.addAggregationField( m_EnrichFieldsName[i], m_MessageEvaluator->getField( field ) );
		}

	}
		
	RoutingAggregationManager::AddRequest( businessCode, RoutingAggregationManager::OptimisticUpdateTrimmed, "BUSINESSMESSAGES" );
}
