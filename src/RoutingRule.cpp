...
RoutingRule::RoutingRule( long id, long schemaId, long sequence, string sessionCode )
{
	...
	try
	{
		...
		// read queue
		m_Queue = RoutingDbOp::GetQueue( myData->getCellValue( 0, "QUEUE" )->getLong() );
		DEBUG( "Queue got" );
		...
		string condition = StringUtil::Trim( myData->getCellValue( 0, "COND_MSG" )->getString() );
		if ( condition.size() > 0 )
			m_Conditions.push_back( RoutingCondition( RoutingCondition::MESSAGE, condition ) );
			
		// add function condition
		condition = StringUtil::Trim( myData->getCellValue( 0, "COND_FUNC" )->getString() );
		if ( condition.size() > 0 )
			m_Conditions.push_back( RoutingCondition( RoutingCondition::FUNCTION, condition ) );
		
		// add metadata condition
		condition = StringUtil::Trim( myData->getCellValue( 0, "COND_META" )->getString() );
		if ( condition.size() > 0 )
